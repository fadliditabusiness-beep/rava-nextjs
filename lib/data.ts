...
export const collectionProducts: CollectionProduct[] = [
  ...
  },
  {
    ...
    price: 30,
    marketplace: {
      shopee: { link: "https://shopee.co.id/product/187765682/41432073064/" },
      tokopedia: { link: "" },
      tiktokShop: { link: "" },
...
function buildMarketplaces(
  ...
  if (pricing) {
    return marketplacePlatforms.map((platform, i) => {
      const key = marketplacePricingKeys[i];
      if (key === "whatsapp") {
        const { beforeDiscount, afterDiscount } = pricing.whatsapp;
        return {
          ...platform,
